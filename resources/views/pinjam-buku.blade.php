        ...
        <div class="container">
            <form action="{{ route('transaksi.store') }}" method="POST" id="transaksiForm">
                ...
                <div class="form-group">
                    ...
                    <input type="date" id="borrow-date" name="borrow_date" min=""
                        onchange="validateDate(); validateReturnDate()" />
                </div>

                <div class="form-group">
                    <label for="return-date">Tanggal Kembali:</label>
                    <input type="date" id="return-date" name="return_date" min="" onchange="validateReturnDate()"
                        disabled />
                </div>
                ...
                <div class="button-container">
                    <a href="/buku ">
                        <button type="button" class="button-primary">Kembali</button>
                    </a>
                    <button type="button" class="button-secondary" onclick="validatePinjam(event)">Pinjam</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        function validatePinjam(event) {
            ...
            success: function(response) {
                ...
                window.location.href = "{{ route('detail-pinjam', $buku->ISBN) }}";
            },
            error: function(xhr, status, error) {
                console.error(error);
