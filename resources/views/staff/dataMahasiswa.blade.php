...
<h2 class="home_page"><b>DATA MAHASISWA</b></h2>
<a href="/staff/tambahMahasiswa">
    <button class="btn btn-warning text-white mt-2 mb-4"><b>Tambah Mahasiswa</b></button>
</a>
    <div class="mb-3">
      ...
      <form>
        <div class="form-row">
          <div class="rounded col-md-12 mb-3 d-flex align-items-center justify-content-between">
            <form action="/staff/dataMahasiswa" method="GET">
                <input type="search" class="form-control mx-2 border-2" id="inputCari" name="search" placeholder="Cari Mahasiswa...">
    ...
    <table class="table table-bordered text-white rounded-table mt-5" style="background: #3A0561">
        <thead>
            <tr class="text-center">
                ...
                <th class="py-2" style="width: 50%;">Nama Mahasiswa</th>
        ...
        <tbody>
            <tr class="text-center">
                ...
                <td class="py-1">{{ $dm->NIM }}</td>
                <td class="py-1">{{ $dm->Nama }}</td>
                <td class="py-1">{{ $dm->email }}</td>
                <td class="py-1 justify-content-between">
                    <div class="d-flex justify-content-between">
                        <a href="/staff/{{ $dm->NIM }}/editMahasiswa">
                            <button class="btn btn-primary btn-sm mx-4">Edit</button>
                        </a>
                        <form action="/staff/{{ $dm->NIM }}/destroyMahasiswa" method="POST" onsubmit="return confirmDelete(event)">
                            @csrf
                            @method('DELETE')
